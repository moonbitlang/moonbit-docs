...
The compiler also reports this warning when `lexscan` is used with a `String`
or `StringView` target. That compatibility form is temporarily accepted, but
new code should use `lexmatch` for in-memory lexical matching.
