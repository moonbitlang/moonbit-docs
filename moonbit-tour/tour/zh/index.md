...
本次教程是互动的！显示的代码是可编辑的，并在您输入时实时编译和求值。使用 `println` 打印的任何内容都将显示在底部部分。教程也默认启用了“值追踪”功能：它将追踪程序运行途中遇到的变量，并将它的值用紫色的提示标记在一边。为了运行 MoonBit 代码，本次教程将 MoonBit 编译为 javascript 并运行，整个过程全部在您的浏览器窗口内完成。
