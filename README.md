...
being applied to the sequence. The former is called _external iterator_ (visible to user) and the
latter is called _internal iterator_ (invisible to user).

The built-in type `Iter[T]` provides internal iterator support for
MoonBit. The type `Iter[T]` is basically a type alias for `((T) -> IterResult)-> IterResult`,
a higher-order function that takes an operation. Almost all
built-in sequential data structures have implemented `Iter`:
...
As seen above, `Iter` provides generic methods like `fold` `filter`, besides
that `map` `each` are also very commonly used. We can implement `Iter` for custom
data structures to take advantages of these methods.

To implement `Iter` for a custom data structure, for example `Bytes`:
...
Underneath the abstractions of `map` and such, is a `self.run()`. Whenever a
`self.run()` is called, the iteration starts. Chaining such methods might be
visually pleasing, but do notice the heavy work deep down.
