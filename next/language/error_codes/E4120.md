...
with `try ... catch`. If the `Result[T, E]` value must escape the local
expression, build it explicitly with `Ok` and `Err` in the `try ... catch`
branches.
...
Or materialize the error to a `Result[T, E]` type with `try ... catch`:
